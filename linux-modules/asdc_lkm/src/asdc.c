...
MODULE_AUTHOR("Yves GUILLEMOT & Alexandre BEAUGY");
...
static int option(int carte, int i)
{
   ...
   switch(carte)
     {
       case 1 : return opt1[i];
       case 2 : return opt2[i];
       case 3 : return opt3[i];
       case 4 : return opt4[i];
       case 5 : return opt5[i];
       case 6 : return opt6[i];
       case 7 : return opt7[i];
     }
...
static int nombre_options(int carte)
{
   ...
   switch(carte)
     {
       case 1 : return n_opt1;
       case 2 : return n_opt2;
       case 3 : return n_opt3;
       case 4 : return n_opt4;
       case 5 : return n_opt5;
       case 6 : return n_opt6;
       case 7 : return n_opt7;
     }
...
static int initialisation_coupleur(struct asdc_varg **pstat,
                                   struct asdc_statics *drstat,
                                   int num, int nb_tamp, int nb_wq);
static void asdcintr_coupleur(struct asdc_varg *dst);
static void bcflux_err(struct asdc_varg *dst,
                       uint16 bc, uint16 flux, uint16 zd,
...
int asdc_fluxbctamp_supprimer(struct asdcbc_tfch **ppsrc,
                              ...
                              struct asdcbc_tfch **pdst);
int asdc_fluxbctamp_ajouter(struct asdc_varg *dst,
                            struct asdcbc_tfch *p, int flux, int zd,
                            struct asdcbc_tfch **pdispo);
...
static int __init init_asdc(void)
{
  ...
  for (k = 0; k < ASDC_MAX_CARTES; k++) asdc_p[k] = NULL;
...
char *asdcinstall(struct abi_pmc_info *info)
{
  ...
  ret = drm_get_handle(PCI_BUSLAYER, ASDC_TEXAS_VENDOR_ID,
                             ASDC_TEXAS_DEVICE_ID, &ti9050_h_drm);
  ...
  ti9050_pci_dev = pci_get_device(ASDC_TEXAS_VENDOR_ID,
                                        ASDC_TEXAS_DEVICE_ID, ti9050_pci_dev);
  ...
  if (ret)
    ...
    { dprintf("TI PCi2050 Bridge Command Reg is \n");
      dprintf("Dev_Ven %x \nStat_Cmd %x \nCls_Rev %x \n",
                                               ti_cmd1, ti_cmd2, ti_cmd3);
      dprintf("Bist_Hd_PLT_CLS %x\n", ti_cmd4);
      dprintf("BAR0 %x \nBAR1 %x \nSblt_sebn_Sbn_Pbn %x \n %x\n",
                                        ti_cmd5, ti_cmd6, ti_cmd7, ti_cmd8 );
    }
  ...
  ret = initialisation_coupleur(&driver_statics->varg[0],
                                driver_statics,
                                info->signal_number_1,
                                info->nombre_tampons_flux_1,
				info->nombre_wq_1);
  ...
  if (driver_statics->bivoie)
    {
      ...
      ret = initialisation_coupleur(&driver_statics->varg[1],
                                    driver_statics,
                                    info->signal_number_2,
                                    info->nombre_tampons_flux_2,
				    info->nombre_wq_2);
      ...
      if (!ret)
        ...
        { /* En cas d'echec, on ne fait rien pour laisser au systeme */
				   ...
				   int nb_wq)
{
  ...
  /* -----------------------------------------------
     ...
     ----------------------------------------------- */
  KPRINTF("Recherche vendor/device 0x%04X/0x%04X : ",
                             ASDC_VENDOR_ID, ASDC_DEVICE_ID);
...
# endif	/* DEVELOPMENT_TEST */
  ...
  sbs1553_pci_dev = pci_get_device(ASDC_VENDOR_ID,
				    ASDC_DEVICE_ID,
				    sbs1553_pci_dev);
...
# ifdef LYNXOS
    ...
    (*pstat)->pbcf = (struct asdcbc_tfch *)
                            sysbrk(sizeof(struct asdcbc_tfch) * nb_tamp);
...
#  if LINUX_VERSION_CODE < KERNEL_VERSION(4,0,0)
   if (request_irq(sbs1553_pci_dev->irq, asdcintr,
                     IRQF_DISABLED | IRQF_SHARED, ASDC_NOM, drstat))
#  else
   if (request_irq(sbs1553_pci_dev->irq, asdcintr,
                     IRQF_SHARED, ASDC_NOM, drstat))
#  endif
     {
...
int asdcopen(struct asdc_statics *driver_statics,
                                             int device, struct file *f )
{
   dprintf("asdcopen(mineur=%d, voie2=%d)\n",
            minor(f->dev), driver_statics->voie2);

   /* Controle de l'existence du coupleur */
   if (minor(f->dev) == 0) return OK;
   if ((minor(f->dev) == 1) && (driver_statics->voie2 == 1)) return OK;

   dprintf(" Echec asdcopen : mineur=%d bivoie=%d\n",
                                minor(f->dev), driver_statics->bivoie);
   pseterr(ENODEV);
   return SYSERR;
...
#  ifdef LYNXOS
     zzprintf("                bvba=0x%08X  h_drm=0x%08X  link_id=%d\n",
                    dst->bvba, dst->h_drm, dst->link_id);
#  elif LINUX
...
#  endif
   ...
   if (cnt < 0) { kkprintf("ASDC : compte d'ITs = %d < 0 !!!\n", cnt);
	 ...
	 switch(L(ptr))
            {  case 0x1 :  zzprintf("ASDC : Timer Data Ready\n");
			 ...
			 SRESET(&dst->sem_finbc);
               ...
               case 0x8 :  zzprintf("ASDC : Interrupt Overflow\n");
			   ...
			   if (cevt )
			     ...
			     { /* Oui ! */
			       ...
			       cevt_signaler(cevt, CEVT_SDCABO,
			                          dst->signal_number,
                              ...
                              struct asdcbc_tfch **pdst)
{
  ...
  if ((*ppsrc == NULL) && (*pdsrc == NULL)) return 0;   /* Rien a faire ! */
...
static void bcflux_err(struct asdc_varg *dst,
                       uint16 bc, uint16 flux, uint16 zd, uint16 code,
