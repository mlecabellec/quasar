...
uint32_t iadr[] = {DIO_INPUT_P0_P3,  DIO_INPUT_P0_P3,  DIO_INPUT_P0_P3,
...
uint32_t oadr[] = {DIO_OUTPUT_P0_P3,  DIO_OUTPUT_P0_P3,  DIO_OUTPUT_P0_P3,
...
int decalage[] = {PPORT0, PPORT1, PPORT2, PPORT3, PPORT4,  PPORT5,
                  PPORT6, PPORT7, PPORT8, PPORT9, PPORT10, PPORT11};

/* Masques des octets associes aux differents ports */
/* dans les registres ci-dessus                     */
int masque[] = {MPORT0, MPORT1, MPORT2, MPORT3, MPORT4,  MPORT5,
...
int listep[] = {LPORT0, LPORT1, LPORT2, LPORT3, LPORT4,  LPORT5,
...
MODULE_AUTHOR("************ GUILLEMOT");
MODULE_DESCRIPTION("Pilote " PCIE6509_NOM " v" __stringify(
    PCIE6509_VERSION) "." __stringify(PCIE6509_REVISION) " du " PCIE6509_DATE);
...
static int initialisation_carte(pcie6509Statics_t **pstat, int numero);
...
irqreturn_t pcie6509Intr(int irq, void *dev_id);
...
static int __init pcie6509Init(void) {
  ...
  if (rslt < 0) {
    ...
    if (rslt == -EINVAL) {
      printLog(DRV_FATAL, "        ==> Le Majeur specifie est hors bornes\n");
    }
    if (rslt == -EBUSY) {
      printLog(DRV_FATAL, "        ==> Le Majeur specifie est indisponible\n");
    }
    return rslt;
  }
  ...
  printLog(DRV_INFO,
           "Debut installation du driver %s v%d.%d   -   Majeur = %d\n",
  ...
  pcie6509Nbre = 0;
  for (k = 0; k < (PCIE6509_MAX_CARTES); k++)
    pAllStat[k] = NULL;
  ...
  if ((cr != -OK) && (cr != -ENODEV)) {
    ...
    printLog(DRV_FATAL, "Abandon de l'installation !\n");
    return -EIO;
  }
  ...
  printLog(DRV_WARNING,
           "Fin installation du driver %s v%d.%d   -   Majeur = %d\n",
           PCIE6509_NOM, PCIE6509_VERSION, PCIE6509_REVISION, majeur);
...
static int initialisation_carte(pcie6509Statics_t **pstat, int numero) {
  ...
  /* --------------------------------------------------
    ...
    -------------------------------------------------- */
  printLog(DRV_INFO,
           "Recherche vendor/device 0x%04X/0x%04X : ", PCIE6509_VENDOR_ID,
           PCIE6509_DEVICE_ID);

  pcie6509_pci_dev =
      pci_get_device(PCIE6509_VENDOR_ID, PCIE6509_DEVICE_ID, pcie6509_pci_dev);

  if (pcie6509_pci_dev == NULL) {
    if (numero)
      printLog(DRV_INFO, "Plus de carte.\n");
    else
      printLog(DRV_INFO, "ECHEC recherche carte !\n");
    return -ENODEV;
  }
  ...
  */
  ...
  for (i = PCIE6509_MAX_CARTES - 1; i >= 0; i--) {
    ...
    if (pAllStat[i] != NULL) {
      ...
      varg = pAllStat[i];
      printLog(DRV_DEBUG, "Debut liberation du device %d [statics = 0x%p]\n", i,
               varg);
...
int pcie6509Open(struct inode *inode, struct file *f) {
  ...
  printLog(DRV_DEBUG, "PCIE6509 [%d] : Appel de open()\n",
           MINOR(inode->i_rdev));
...
int pcie6509Close(struct inode *inode, struct file *f) {
  printLog(DRV_DEBUG, "PCIE6509 [%d] : Appel de close()\n",
           MINOR(inode->i_rdev));
  return -OK;
}
