...
pcie6509UnlockedIoctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
        ...
        case PCIE6509_SET_BIT : {
            ...
            printLog(DRV_DEBUG,
                     "PCIE6509_SET_BIT [%d,%d] v=0x%X  0x%02X --> ",
                     port, data.bit, v, pstat->bit[port]);
        ...
        case PCIE6509_GET_VER : {
            ...
            p = __stringify(PCIE6509_VERSION)
                        "." __stringify(PCIE6509_REVISION);
