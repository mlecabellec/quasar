...
int cevtioctl(struct cevt_statics *dst, struct file *f, int commande,
              char *arg);
...
static char __init *cevtinstall(struct cevtinfo *info) {
  ...
  driver_statics = (struct cevt_statics *)sysbrk(sizeof(struct cevt_statics));
  ...
  driver_statics->pbcfev =
      (struct cevt_tfch *)sysbrk(sizeof(struct cevt_tfch) * info->nb_tamp);
...
int cevtopen(struct cevt_statics *driver_statics, int device, struct file *f) {
...
int cevtselect(struct cevt_statics *dst, struct file *f, int which,
               struct sel *ffs) {
...
int cevt_existence(int numero_cevt) {
  ...
  /* Numero CEVT valide ? */
  if ((numero_cevt < 1) || (numero_cevt > CEVT_MAX_DEVICES))
    return 0;
...
int cevt_signaler(int numero_cevt,
  ...
  dprintf("CEVT_signaler %d  ES=%d/%d evt=0x%08X, data=0x%x\n", numero_cevt,
          nature_es, numero_es, evenement, donnee);
  ...
  spin_lock_irqsave(&dst->lock_attente,
                    ps); /******* Debut section critique *******/
  ...
  if ((nature_es != CEVT_EXT) && (evenement == CEVT_AVORTE)) {
  ...
  if (dst->rsel_sem)
    SSIGNAL(dst->rsel_sem);
#endif /* LYNXOS */

  spin_unlock_irqrestore(&dst->lock_attente,
                         ps); /******* Fin section critique *******/
...
struct dldd entry_points = {
    cevtopen,   cevtclose, cevtread,    cevtwrite,     /* stub */
    cevtselect, cevtioctl, cevtinstall, cevtuninstall, /* stub */
    (char *)0};
#endif /* LYNXOS */
