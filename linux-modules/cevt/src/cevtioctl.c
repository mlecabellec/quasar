...
/* -----------------------------------------------
   ...
   ----------------------------------------------- */
#ifdef LYNXOS
  int cevtioctl(struct cevt_statics *lynx_statics,
                    struct file *f, int commande, char *arg )
#elif LINUX
#ifdef OLDIOCTL
  int cevtioctl(struct inode *inode,
     ...
     case CEVT_LIRE :
        {
          ...
          spin_unlock_irqrestore(&dst->lock_attente, s);	/* Fin de la section critique */
