...
unsigned char*	lpszXsvfData = NULL;
unsigned long   ulRemindData = 0;
...
void setPort(DEVICE_EXTENSION *pDe, short p, short val)
{
    ...
    if (p==TCK) {
        g_iTCK = val;
    PcigSetJTagPort(pDe, g_iTCK, g_iTMS, g_iTDI);
    }
}
...
{
    setPort(pDe, TCK, 0);  /* set the TCK port to low  */
    setPort(pDe, TCK, 1);  /* set the TCK port to high */
}
...
unsigned char readTDOBit(DEVICE_EXTENSION *pDe)
{
    ...
    return((unsigned char)PcigGetJTagTDO(pDe));
}
