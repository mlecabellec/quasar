...
*               v4.06   - Fix xsvfGotoTapState for retry traD33AD2nsition.
...
extern unsigned long ulRemindData;
...
void xsvfPrintLenVal( lenVal *plv )
{
    ...
    if ( plv )
    {
        ...
        for ( i = 0; i < plv->len; ++i )
        {
            printk( "%02x", ((unsigned int)(plv->val[ i ])) );
        }
    }
}
#endif  /* DEBUG_MODE */
                    ...
                    int     iExitShift )
{
    ...
    while ( lNumBits )
    {
        ...
        for ( i = 0; ( lNumBits && ( i < 8 ) ); ++i )
        {
            ...
            setPort(pDe, TDI, (short)(ucTdiByte & 1) );
            ucTdiByte   >>= 1;
...
int xsvfInitialize( SXsvfInfo** pXsvfInfo )
{
    ...
    *pXsvfInfo = vmalloc(sizeof(SXsvfInfo));
...
*****************************************************************************/
int xsvfExecute(DEVICE_EXTENSION *pNewDe)
{
    SXsvfInfo   *pxsvfInfo = NULL;
    int		iErrorCode;
    ...
    {
        XSVFDBG_PRINTF1( 0, "%s\n", xsvf_pzErrorName[
                         ( pxsvfInfo->iErrorCode < XSVF_ERROR_LAST )
                         ? pxsvfInfo->iErrorCode : XSVF_ERROR_UNKNOWN ] );
        XSVFDBG_PRINTF2( 0, "ERROR at or near XSVF command #%ld.  See line #%ld in the XSVF ASCII file.\n",
                         pxsvfInfo->lCommandCount, pxsvfInfo->lCommandCount );
        printk( "%s\n", xsvf_pzErrorName[
                         ( pxsvfInfo->iErrorCode < XSVF_ERROR_LAST )
                         ? pxsvfInfo->iErrorCode : XSVF_ERROR_UNKNOWN ] );
        printk( "ERROR at or near XSVF command #%ld.  See line #%ld in the XSVF ASCII file.\n",
                         pxsvfInfo->lCommandCount, pxsvfInfo->lCommandCount );
	printk("RemindData = %ld\n", ulRemindData);
    }
    else
    {
        XSVFDBG_PRINTF( 0, "SUCCESS - Completed XSVF execution.\n" );
        printk("SUCCESS - Completed XSVF execution.\n" );
    }
    iErrorCode = pxsvfInfo->iErrorCode;
    xsvfCleanup( pxsvfInfo );
    pxsvfInfo = NULL;
    pDe = NULL;
    return( XSVF_ERRORCODE(iErrorCode) );
}
