// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/16AI64SSA/samples/irq/irq_1.c $
// $Rev: 51334 $
// $Date: 2022-07-11 17:32:15 -0500 (Mon, 11 Jul 2022) $
...
//*****************************************************************************
static int _irq1_in_buf_thr_l2h_test(int fd)
{
	int	errs;

	gsc_label("Buffer Threshold Low-to-High");
	errs	= irq_wait_test(fd, 0, AI64SSA_WAIT_GSC_IN_BUF_THR_L2H, 0, _low_2_high);
	return(errs);
}
...
//*****************************************************************************
static int _irq1_in_buf_thr_h2l_test(int fd)
{
	int	errs;

	gsc_label("Buffer Threshold High-to-Low");
	errs	= irq_wait_test(fd, 0, AI64SSA_WAIT_GSC_IN_BUF_THR_H2L, 0, _high_2_low);
	return(errs);
}
...
//*****************************************************************************
static int _irq1_in_buf_error_test(int fd)
{
	s32	buf_err	= 0;
	int	errs;
	...
	else
	{
		...
		gsc_label("Overflow");
		errs	+= irq_wait_test(fd, 0, AI64SSA_WAIT_GSC_BUF_ERROR, 0, _error_overflow);

		gsc_label("Underflow");
		errs	+= irq_wait_test(fd, 0, AI64SSA_WAIT_GSC_BUF_ERROR, 0, _error_underflow);
...
int irq_1_test(int fd)
{
	...
	errs	+= _irq1_in_buf_thr_l2h_test(fd);
	errs	+= _irq1_in_buf_thr_h2l_test(fd);
	errs	+= _irq1_in_buf_error_test(fd);
