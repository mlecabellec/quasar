// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/16AI64SSA/samples/rxrate/main.c $
// $Rev: 54967 $
...
#define	DATA_PACK			AI64SSA_DATA_PACKING_ENABLE
#define	SCAN_MARK			AI64SSA_SCAN_MARKER_ENABLE
...
static int _parse_args(int argc, char** argv, args_t* args)
{
	const gsc_arg_item_t	list[]	=
	{
		...
		{	GSC_ARG_S32_RANGE,	&args->fsamp,			"-fsamp",	{ 1, 200000 },			"Sample data at # samples per second (default: max)."	},
		{	GSC_ARG_S32_FLAG,	&args->data_packing,	"-dp",		{ DATA_PACK },			"Enable Data Packing."									},
		...
		{	GSC_ARG_S32_MIN,	&args->rx_mb,			"-r",		{ 1 },					"Receive # megabytes (default: 4)."						},
		{	GSC_ARG_S32_RANGE,	&args->seconds,			"-s",		{ 1, 3600 },			"Read data for at least # seconds (default: 15)."		},
		{	GSC_ARG_S32_FLAG,	&args->scan_marker,		"-sm",		{ SCAN_MARK },			"Enable the data packing Scan Marker."					},
	...
	{
		/* name	*/	"rxrate",
		/* desc	*/	"Receive Rate Performance Tests",
	...
	args->data_packing	= AI64SSA_DATA_PACKING_DISABLE;
	args->fsamp			= 200000;
	args->io_mode		= GSC_IO_MODE_PIO;
	args->rx_mb			= 4;
	args->scan_marker	= AI64SSA_SCAN_MARKER_DISABLE;
	args->seconds		= 15;
