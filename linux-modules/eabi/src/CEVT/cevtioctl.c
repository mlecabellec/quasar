...
/* -----------------------------------------------
   ...
   --                                           --
   ----------------------------------------------- */
long cevtioctl(int vfxNum, int asdcNum, struct file *fp, unsigned int cmd,
               unsigned long arg) {
  ...
  case CEVT_LETAT: /* Lecture des indicateurs d'etat du pilote */
                   /* (pour debug uniquement)                  */
    {
      ...
      if ((e->numero < 1) || (e->numero > CEVT_MAX_DEVICES))
        return ENODEV;
  ...
  case CEVT_SIGNALER: {
    ...
    if ((a->numero < 1) || (a->numero > CEVT_MAX_DEVICES))
      return ENODEV;
