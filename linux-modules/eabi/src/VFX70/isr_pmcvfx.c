...
12/01/2015  YG  Mise en oeuvre du verrouillage des E/S PCIe
...
#define L2(a) lecturePCIe(unit, 1, (int *)(pvargs[unit]->base2 + (a)))
#define E2(a, v) ecriturePCIe(unit, 1, (v), (int *)(pvargs[unit]->base2 + (a)))
...
/// printk("DIO ISR HwdAdr=%lX t=%X e=%X s=%X\n",
///       (unsigned long)map_ptr, type, enable, status);
    if (type != 0) {
        printk("type = 0x%0X : relecture forcee\n", type);
        type = readw( (word*)&map_ptr->type_reg); /* interrupt type */
        printk("type (2) = 0x%0X\n", type);
    }
