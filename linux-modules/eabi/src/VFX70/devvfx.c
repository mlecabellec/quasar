...
12/01/2015  YG  Ajout bigEmuabiLock pour essai suppression crash ISF.
                - Dans cette version (monobloc) du driver, ce verrou doit etre
                  utilise pour ne pas effectuer plusieurs transferts PCIe
                  simultanement.
...
#define BUILDING_FOR_KERNEL /* controls conditional inclusion in file          \
                               pmcvxmulticommon.h */
...
MODULE_AUTHOR("************ Anonymized (Astrium)");
MODULE_DESCRIPTION("Pilote " VFX_NOM " v" __stringify(
    VFX_VERSION) "." __stringify(VFX_REVISION) " du " VFX_DATE);
...
#ifdef OLDIOCTL
int vfx_ioctl(struct inode *inode, struct file *fp, unsigned int cmd,
              unsigned long arg);
#else
long vfx_ioctl(struct file *fp, unsigned int cmd, unsigned long arg);
#endif
...
static ssize_t write(struct file *fp, const char *buf, size_t length,
                     loff_t *offset) {
  unsigned long adata, ldata;
...
#define tag(x)                                                                 \
  ecriturePCIe(unit, 1, 0xF000 | (((~(x)) << 1) & 0xE),                        \
               (int *)(pvargs[unit]->base2 + (0x9094)))

static int apmcvfxboard_handler(int irq, void *did, struct pt_regs *cpu_regs) {
  ...
  spin_lock(&((vfxvarg_t *)did)->bigEmuabiLock);
  ...
  if (etat) {
    ...
    //      printk("--- IT : etat=0x%X : Inhibition\n", etat);
    //     printk("I %X\n", etat);
  } else {
    tag(7);
    // printk("tag(7)\n");
    spin_unlock(&((vfxvarg_t *)did)->bigEmuabiLock);
    return IRQ_NONE;
  }
...
int init_module(void) {
  ...
  for (i = 0, j = 0; i < MAX_PMCS; i++) {
    ...
    if (pvfx) {
      ...
      pmc_address[i] = (unsigned long)ioremap(
          pmc_address[i], PMC_VX_BAR0_MAP_SIZE); /* no cache! */
#else
      pmc_address[i] = (unsigned long)ioremap_nocache(
          pmc_address[i], PMC_VX_BAR0_MAP_SIZE); /* no cache! */
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0)
      pmc_address1[i] = (unsigned long)ioremap(
          pmc_address1[i], PMC_VX_BAR1_MAP_SIZE); /* no cache! */
#else
      pmc_address1[i] = (unsigned long)ioremap_nocache(
          pmc_address1[i], PMC_VX_BAR1_MAP_SIZE); /* no cache! */
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0)
      pmc_address2[i] = (unsigned long)ioremap(
          pmc_address2[i], PMC_VX_BAR2_MAP_SIZE); /* no cache! */
#else
      pmc_address2[i] = (unsigned long)ioremap_nocache(
          pmc_address2[i], PMC_VX_BAR2_MAP_SIZE); /* no cache! */
#endif
      // printk("VFX70 - Apres ioremap_nocache() (x3)\n");

      if ((pmc_address[i] != 0) && (pmc_address1[i] != 0) &&
          (pmc_address2[i] != 0)) {
        ...
        spin_lock_init(&pvargs[i]->bigEmuabiLock);
        pvargs[i]->bigLockCount = 0;

        printk("VFX70 : Connexion IRQ %d au device #%d\n", board_irq[i], i);
        ret_val = request_irq(board_irq[i], (irq_handler_t)apmcvfxboard_handler,
        ...
        printk("%s mapped   BAR0=%08lX BAR1=%08lX\n"
               ...
               (unsigned long)pmc_address1[i], (unsigned long)pmc_address2[i],
               board_irq[i]);
  ...
  if (j) { /* found at least one device */
    ...
    if (ret_val < 0) {
      ...
      /* L'initialisation de la partie VFX70 du driver EMUABI s'est */
      /* bien passee : initialisation de la partie ASDC du driver.  */
      cr = init_asdc();

      if (cr) {
        /* Aucune structure logicielle associee a un materiel */
        /* de type ASDC n'a pu etre installee.                */
        /* La partie deja installee du driver n'a aucune      */
        /* raison d'etre et doit etre desinstallee.           */
        cleanup_module();
        return -ENODEV;
      } else {
    ...
    for (i = 0; i < MAX_PMCS; i++) {
      if (pmc_address1[i]) {
        ...
        printk("%s unmapped BAR0=%08lX BAR1=%08lX BAR2=%08lX\n", devnamebuf,
               (unsigned long)pmc_address[i], (unsigned long)pmc_address1[i],
...
int VFX70_getNumberOfCards(void) { return countOfVfx70; }
...
void VFX70_getBaseAddresses(unsigned int vfxNum, void **pcibar0, void **pcibar1,
                            void **pcibar2) {
  /* Controle de la validite du numero de l'instance VFX70 */
...
int VFX70_registerUserDriver(unsigned int vfxNum, unsigned int userNum,
                             long (*up_ioctl)(int vfx, int asdc,
                                              struct file *fp, unsigned int cmd,
                                              unsigned long arg),
                             void (*up_isr)(int unit)) {
  /* Controle de la validite du numero de l'instance VFX70 */
  if (vfxNum >= MAX_PMCS)
    return -ENODEV;
  if (pvargs[vfxNum] == NULL)
    return -ENODEV;
