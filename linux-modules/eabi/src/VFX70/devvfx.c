...
#define tag(x) ecriturePCIe(unit, 1, 0xF000 | (((~(x)) << 1) & 0xE), (int *)(pvargs[unit]->base2 + (0x9094)))
...
init_module( void )
{
    ...
    for( i = 0, j = 0; i < MAX_PMCS; i++ ) {
        ...
        if( pvfx ) {
            ...
            pmc_address[i] = (unsigned long)ioremap( pmc_address[i], PMC_VX_BAR0_MAP_SIZE); /* no cache! */
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0)
            pmc_address1[i] = (unsigned long)ioremap( pmc_address1[i], PMC_VX_BAR1_MAP_SIZE); /* no cache! */
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0)
            pmc_address2[i] = (unsigned long)ioremap( pmc_address2[i], PMC_VX_BAR2_MAP_SIZE); /* no cache! */
#else
