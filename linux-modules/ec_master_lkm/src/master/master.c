...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 11, 0) && LINUX_VERSION_CODE < KERNEL_VERSION(5, 1, 0)
        ...
        )
{
    ...
    master->forcedSlavesSyncs = NULL;
    master->countForcedSlaves = 0;
#endif
...
#ifdef EC_MASTER_SYNC_CONFIG_CAP
	if(master->forcedSlavesSyncs)
	{
		for(i=0; i<master->countForcedSlaves; ++i)
		{
			if(master->forcedSlavesSyncs[i].forced_syncs)
			{
				kfree(master->forcedSlavesSyncs[i].forced_syncs);
			}
			if(master->forcedSlavesSyncs[i].isConfigured)
			{
				kfree(master->forcedSlavesSyncs[i].isConfigured);
			}
		}
		kfree(master->forcedSlavesSyncs);
	}
...
void ec_master_eoe_start(ec_master_t *master /**< EtherCAT master */)
{
    ...
    sched_set_normal(master->eoe_thread, 0);
#else
...
#ifdef RING_FEATURE_ENABLED 
		EC_MASTER_ERR(master, "Warning detected during topology scanning. Ignored as ring topology is enabled.\n");
                return 0;
#else
                return -1;
#endif     
