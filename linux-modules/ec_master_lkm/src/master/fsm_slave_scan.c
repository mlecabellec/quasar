...
void ec_fsm_slave_scan_state_sii_data(ec_fsm_slave_scan_t *fsm /**< slave state machine */)
{
	...
	for(i=0; i < slave->master->countForcedSlaves; ++i)
	{
		if(slave->ring_position == slave->master->forcedSlavesSyncs[i].slave_index)
		{
			...
			slave->sii.syncs=kmalloc(slave->master->forcedSlavesSyncs[i].forced_sync_count * sizeof(ec_sync_t), GFP_KERNEL);
			for(j=0; j < slave->master->forcedSlavesSyncs[i].forced_sync_count; ++j)
			{
				ec_sync_init(&(slave->sii.syncs[j]),slave);
				slave->sii.syncs[j].physical_start_address=slave->master->forcedSlavesSyncs[i].forced_syncs[j].physical_start_address;
				slave->sii.syncs[j].default_length=slave->master->forcedSlavesSyncs[i].forced_syncs[j].default_length;
				slave->sii.syncs[j].control_register=slave->master->forcedSlavesSyncs[i].forced_syncs[j].control_register;
				slave->sii.syncs[j].enable=slave->master->forcedSlavesSyncs[i].forced_syncs[j].enable;
			}
			slave->sii.sync_count = slave->master->forcedSlavesSyncs[i].forced_sync_count;
			EC_MASTER_INFO(slave->master, "Using forced sync configuration for slave %d", slave->ring_position);
			break;
		}
	}
#endif
