        ...
        )
{
...
#endif
    // ASTRE fix: dev_addr is read-only, so use an array with eth_hw_addr_set function
    for (i = 0; i < ETH_ALEN; i++) {
        ((u8*)eoe->dev->dev_addr)[i] = i | (i << 4);
    }
    ...
    *priv = eoe;
    ...
    ((u8*)eoe->dev->dev_addr)[ETH_ALEN - 1] = (uint8_t) eoe->dev->ifindex;
    return 0;
