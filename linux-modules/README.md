...
|---|---|---|
| `ai64_lkm` | General Standards AI64 family (PMC/PCI) | Data acquisition card driver. |
| `ai64ll_lkm` | Concurrent Real-Time AI64LL family | Low-latency analog input card driver. |
| `apmc424_lkm` | Acromag PMC424 (PMC) | Digital I/O and Counter/Timer module. |
| `asdc_lkm` | SBS Technologies MIL-STD-1553 family (PMC) | MIL-STD-1553 interface (BC, RT, BM). |
| `cevt_lkm` | N/A (pseudo-driver) | Aggregates hardware events for user-space. |
| `digicool_lkm` | ADAS "digicool" cards with PLX bridge chips (PCI/PCIe) | Generic driver framework for PCI/PCIe cards. |
| `dummy_lkm` | N/A | Template for creating new kernel modules. |
| `eabi_lkm` | Acromag PMC-VFX70 (PMC) | Emulates a MIL-STD-1553 interface using an FPGA. |
| `ec_generic_lkm` | Any standard Ethernet card | Generic Ethernet device driver for IgH EtherCAT master. |
| `ec_master_lkm` | N/A | IgH EtherCAT master implementation. |
| `gsc16ai_lkm` | General Standards 16AI64SSA (PCI/PCIe) | 64-channel simultaneous sampling analog input. |
| `gsc16ao_lkm` | General Standards 16AO family (PCI/PMC) | Analog output cards with up to 16 channels. |
| `opto16x16_lkm` | General Standards OPTO16X16 (PCIe) | 16 isolated digital inputs and 16 isolated digital outputs. |
| `pcie6509_lkm` | National Instruments PCIe-6509 | 96-channel digital I/O card driver (by Concurrent). Includes watchdog and input filters. |
| `pcie6509_simusol_lkm`| National Instruments PCIe-6509 | 96-channel digital I/O card driver (by Astrium). |
| `rfm2g_lkm` | GE/Abaco RFM2g family (PCI, PMC, PCIe, VME) | 2 Gbit/s Reflective Memory network driver. |
| `tsyncpci_lkm` | Spectracom TSync family (PCI, PCIe, cPCI) | High-precision time synchronization (GPS, IRIG, etc.). |
| `uceipci_lkm` | Abaco/Condor UCEI family (PCIe) | Universal driver for MIL-STD-1553 and ARINC-429 cards. |
| `vfx70_lkm` | Acromag PMC-VFX70 (PMC) | User-configurable FPGA module driver. Used by `eabi_lkm`. |
...
*   **Features:** "Concentrateur d'EVenemenTs" (Event Concentrator). Software layer that works in conjunction with other hardware drivers, such as `asdc_lkm`. Aggregates events from various hardware drivers and provides a single, unified interface for user-space applications to consume these events. Tightly coupled with the `asdc_lkm` driver.
...
*   **Features:** Part of the IgH EtherCAT Master stack. Implements the high-level EtherCAT master protocol and logic. Works in conjunction with the `ec_generic_lkm` module. Allows a standard computer running Linux to act as a full-featured EtherCAT master, capable of controlling a network of EtherCAT slave devices.
...
## pcie6509_simusol_lkm

*   **Hardware:** National Instruments PCIe-6509 digital I/O card.
*   **Features:** Third-party Linux driver (from Astrium ST). 96 bidirectional digital I/O lines. TTL/CMOS compatibility. Each channel can be individually configured as an input or output. This is a separate, independent implementation from `pcie6509_lkm`.
...
*   **Features:** 64-bit, 100MHz PCI-X bus. 64 I/O lines or 32 LVDS lines. Dependency for the `eabi_lkm` module to emulate a MIL-STD-1553 interface.
