 ...
 *
 * Copyright 2020 ArianeGroup. All rights reserved.
...
#define BOARD_EXISTS                                                                      \
    (ai64ssa_init_util(0) == 0 && ai64ssa_count_boards(0, &TestGsc16Ai::_BoardCount) == 0 \
...
public:
    ...
    int ai64ssa_config_ai5V(int _fileDesc, int index, int verbose, s32 fsamp) {
        ...
        errs += ai64ssa_rx_io_mode(_fileDesc, index, verbose, GSC_IO_MODE_BMDMA, nullptr);
        errs += ai64ssa_rx_io_overflow(_fileDesc, index, verbose, AI64SSA_IO_OVERFLOW_CHECK, nullptr);
        errs += ai64ssa_rx_io_timeout(_fileDesc, index, verbose, AI64SSA_IO_TIMEOUT_DEFAULT, nullptr);
        errs += ai64ssa_rx_io_underflow(_fileDesc, index, verbose, AI64SSA_IO_UNDERFLOW_CHECK, nullptr);
        errs += ai64ssa_ai_range(_fileDesc, index, verbose, AI64SSA_AI_RANGE_0_5V, nullptr);
        errs += ai64ssa_ai_buf_thr_lvl(_fileDesc, index, verbose, 250000, nullptr);
        errs += ai64ssa_ai_mode(_fileDesc, index, verbose, AI64SSA_AI_MODE_SINGLE, nullptr);
        errs += ai64ssa_burst_size(_fileDesc, index, verbose, 1, nullptr);
        errs += ai64ssa_burst_src(_fileDesc, index, verbose, AI64SSA_BURST_SRC_DISABLE, nullptr);
        ...
        errs += ai64ssa_data_format(_fileDesc, index, verbose, AI64SSA_DATA_FORMAT_OFF_BIN, nullptr);
        errs += ai64ssa_data_packing(_fileDesc, index, verbose, AI64SSA_DATA_PACKING_DISABLE, nullptr);
        errs += ai64ssa_irq0_sel(_fileDesc, index, verbose, AI64SSA_IRQ0_INIT_DONE, nullptr);
        errs += ai64ssa_irq1_sel(_fileDesc, index, verbose, AI64SSA_IRQ1_NONE, nullptr);
        errs += ai64ssa_fsamp_ai_compute(_fileDesc, index, verbose, fsamp, &_src_a, &_src_b, &_nrate_a, &_nrate_b,
        ...
        errs += ai64ssa_scan_marker(_fileDesc, index, verbose, AI64SSA_SCAN_MARKER_DISABLE, nullptr);
    ...
    void testWait() {
        ...
        if (ai64ssa_open_util(deviceIndex, 0, -1, 1, &fileDesc) != 0 || fileDesc < 0) {
        ...
        CPPUNIT_ASSERT(ai64ssa_chan_active(fileDesc, -1, verbose, AI64SSA_CHAN_ACTIVE_0_63, nullptr) == 0);
    ...
    void testRead() {
        ...
        CPPUNIT_ASSERT(ai64ssa_chan_active(fileDesc, -1, verbose, AI64SSA_CHAN_ACTIVE_0_15, nullptr) == 0);

        int qty = 0;
        CPPUNIT_ASSERT(ai64ssa_query(fileDesc, -1, 0, AI64SSA_QUERY_CHANNEL_QTY, &qty) == 0);
        ...
        while (true) {  // for (uint32_t idx = 0; idx < 2000; idx++) {
            ...
            int length = ai64ssa_read(fileDesc, reinterpret_cast<uint8_t*>(values), sizeof(values));
            LOG_INFO("Reda2");
            if (length < 0) {
                CPPUNIT_ASSERT(ai64ssa_ai_buf_clear(fileDesc, -1, verbose) == 0);
            }
            else {
                double value = getDoubleValueFromInt16(values[8], AI64SSA_AI_RANGE_0_5V);
                if (std::abs((double)(value - prevValue)) > 2) {
                    LOG_INFO("Value read.......: %lf", value);
                    prevValue = value;
                }
    ...
    double getDoubleValueFromInt16(int receivedInt, int range) {
        ...
        switch (range) {
            ...
            case (AI64SSA_AI_RANGE_10V):
                ...
                break;
            default:
                LOG(error, "DevGsc16ai  tried to convert voltage from an unknown voltage range.");
                break;
        }
        return voltage;
    }
    ...
    double readDMA(int index) {
        ...
        errs += ai64ssa_rx_io_overflow(_fileDesc, -1, 0, AI64SSA_IO_OVERFLOW_IGNORE, nullptr);
        errs += ai64ssa_rx_io_timeout(_fileDesc, -1, 0, AI64SSA_IO_TIMEOUT_DEFAULT, nullptr);
        errs += ai64ssa_rx_io_underflow(_fileDesc, -1, 0, AI64SSA_IO_UNDERFLOW_IGNORE, nullptr);
        ...
        errs += ai64ssa_burst_src(_fileDesc, -1, 0, AI64SSA_BURST_SRC_DISABLE, nullptr);
        errs += ai64ssa_data_packing(_fileDesc, -1, 0, AI64SSA_DATA_PACKING_DISABLE, nullptr);
        // LOG_INFO("Read1_2");
        // errs	+= ai64ssa_fsamp_ai_compute	(_fileDesc, -1, 0, fsamp, &_src_a, &_src_b, &_nrate_a, &_nrate_b,
        // &_enable_a, &_enable_b, nullptr);
        ...
        errs += ai64ssa_scan_marker(_fileDesc, -1, 0, AI64SSA_SCAN_MARKER_DISABLE, nullptr);
        ...
        if (ret < 0) {
            errs++;
            CPPUNIT_FAIL(String::Format("ERROR: ai64ssa_read() failure, returned %d", ret));
        }
