// $URL: http://subversion:8080/svn/gsc/trunk/drivers/gsc_common/driver/linux/os_kernel_5.c $
// $Rev: 53838 $
...
// variables ******************************************************************
	...
	static const struct file_operations	_proc_ops	=
	{
		.open	= _proc_open,
		.read	= seq_read
	};
...
int os_module_count_inc(void)
{
	...
	if (ret)
	{
		printf(	"%s: %d. %s: module incriment failed.\n",
				DEV_NAME,
				__LINE__,
				__FUNCTION__);
	}
...
static int _proc_show(struct seq_file* sf, void* vp)
{
	...
	for (;;)	// A convenience loop.
	{
		...
		{
			ret	= -ENOMEM;
			printf(	"%s: %d. %s: memory allocation failed: %d bytes\n",
					DEV_NAME,
					__LINE__,
					__FUNCTION__,
					(int) bytes);
...
int os_proc_start_detail(void)
{
	...
	{
		ret	= -ENODEV;
		printf(	"%s: %d. %s: proc_create() failure.\n",
				DEV_NAME,
				__LINE__,
				__FUNCTION__);
	}
