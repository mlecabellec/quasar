// $URL: http://subversion:8080/svn/gsc/trunk/drivers/LINUX/OPTO16X16/OPTO16X16_Linux_1.x.x.x_GSC_DN/samples/dout/main.c $
// $Rev: 53721 $
...
static int _parse_args(int argc, char** argv, args_t* args)
{
	const gsc_arg_item_t	list[]	=
	{
		...
		{	GSC_ARG_S32_RANGE,	&args->ms_wait,			"-ms",		{ 0, 1000 },	"Wait # ms between writes (default: 250)."				},
		{	GSC_ARG_S32_MIN,	&args->test_limit,		"-n",		{ 1 },			"Continue testing for up to # iterations."				},
		{	GSC_ARG_S32_FLAG,	&args->pause,			"-p",		{ 1 },			"Pause after each individual bit test."					},
		{	GSC_ARG_S32_RANGE,	&args->repeat,			"-r",		{ 1, 1000 },	"Repeat the output pattern # times."					},
	...
	{
		/* name	*/	"dout",
		/* desc	*/	"Demonstrate digital output.",
