...
extern long
pcie6509UnlockedIoctl(struct file *fp, unsigned int cmd, unsigned long arg);

extern void
initPrintLog(int destination, int niveau);

extern void
printLog(const int msglevel, const char *format, ...);

/* Lecture d'un port en entree */
extern uint8_t lirePortEntree(pcie6509Statics_t * pstat, int port);

/* Ecriture d'un port en sortie */
extern void ecrirePortSortie(pcie6509Statics_t * pstat,
                             int port, uint8_t valeur);

/* Ecriture des directions des bits d'un port */
extern void ecrirePortDirection(pcie6509Statics_t * pstat,
                                int port, uint8_t valeur);
