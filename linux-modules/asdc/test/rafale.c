 ...
 *
 * Emission d'une "rafale" de donnees a destination dun abonne
 * (pour mise au point de la simu d'abonnes sous IT)
 *
 * History
 *
...
main(int argc, char **argv)
{
  ...
  int nbmts;	/* Nombre de tampons prevus */
  ...
  /* 7 - Transfert RTBC 3 et rebouclage de la trame */
...
printf("--- Fin programmation periodicite de la trame ---\n"); fflush(stdout);    
  ...
  tamp_s = (struct asdcbc_tf *) calloc(nbmts, sizeof(struct asdcbc_tf));
  if (tamp_s == NULL)
    { fprintf(stderr, "Echec allocation memoire (%d octets)\n",
                       nbmts * sizeof(struct asdcbc_tf));
      exit(-1);
    } 
  ...
  j = 0;
  for (i=0, k=0; i<(nbmts/2); i++, k+=2)
    { tamp_s[k].type = BC_BCRT;
      tamp_s[k].cmd1 = (9 << 11) + (5 << 5) + 2;	/* RT9,5 n=2 */
      tamp_s[k].d[0] = j++;
      tamp_s[k].d[1] = j++;
      
      tamp_s[k+1].type = BC_BCRT;
      tamp_s[k+1].cmd1 = (9 << 11) + (6 << 5) + 3;	/* RT9,6 n=3 */
      
   // Decommenter ci-dessous pour creer une erreur de coherence flux/tampon   
   // if (i==52) tamp_s[k].cmd1 = (9 << 11) + (6 << 5) + 4;
  ...
  /* Ecriture de nbmts tampons en sortie du flux */
  /* (sauf les tampons materiels)                */
