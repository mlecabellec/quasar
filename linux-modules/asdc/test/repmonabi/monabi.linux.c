...
/*                                                                 */
/*   Version 3.2 : - Adaptation a abi-pmc2 sous Linux              */
/*                                    modif. le 3 septembre 2002   */
/*                           derniere modif. le  23 janvier 2003   */
/*******************************************************************/
...
#define VERSION_MINEUR	2
#define DATE    "3/9/2002"
...
char **argv;
{
  ...
  if (device == NULL) device = "/dev/asdc";
  if (fprm == NULL) fprm = "";
  if (fichier == NULL)
       { fichier = "espion.dat";
         fo = 1;	/* Indique qu'il faut ouvrir un fichier */
  ...
  fprintf(stderr,
         ...
         for (s=0; s<32; s++)     /* Sous-adresse */
             for (d=0; d<2; d++)   /* Direction du transfert */
                  { v.adresse = a;
                    v.sous_adresse = s;
                    v.direction = d;
	//    fprintf(stderr,"a=%d s=%d d=%d\n", a, s, d); fflush(stderr);
		       if(ioctl(fasdc, ASDCVALESPS, &v) == -1)
   ...
   /* (taille suffisante pour plus grand tampon possible) */
	fprintf(stderr,"avant calloc\n");
	fflush(stderr);

   zone = (MOT *) calloc(32768, sizeof(MOT));
	fprintf(stderr,"apres calloc\n");
	fflush(stderr);
   if (zone == NULL)
  ...
  if (nes == 1)
   ...
   while (!terminer)
	...
	/* YG - 3/9/2002 : Petit bricolage pour contourner pb apparent */
	/* d'erreur de tampon ...                                      */
	if (terminer) sleep(2);
	...
	atampon = LRAM(fasdc, M2PTR);
        ...
        if (r==-1) /* Alors : debordement du tampon ! */
