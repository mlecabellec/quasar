...
{
    unsigned int doFullStateMachine = 0;
    ec_slave_state_t currentState = fsm->slave->current_state & 0x0F;

    switch (fsm->slave->requested_state) {
    case EC_SLAVE_STATE_SAFEOP:
        EC_SLAVE_DBG(fsm->slave, 1, "Go safeop...\n");
        switch (currentState) {
        case EC_SLAVE_STATE_OP:
            ec_fsm_slave_config_enter_safeop(fsm);
            break;
        case EC_SLAVE_STATE_PREOP:
            // need to get the syncmanager for the safeop state.
            ec_fsm_slave_config_enter_boot_preop(fsm);
            break;
        default: doFullStateMachine = 1; break;
        }
        break;
    case EC_SLAVE_STATE_OP:
        switch (currentState) {
        case EC_SLAVE_STATE_SAFEOP:
            EC_SLAVE_DBG(fsm->slave, 1, "Go op...\n");
    ...
    case EC_SLAVE_STATE_PREOP:
        ...
        case EC_SLAVE_STATE_OP:
        case EC_SLAVE_STATE_SAFEOP:
            EC_SLAVE_DBG(fsm->slave, 1, "Go preop...\n");
            ec_fsm_slave_config_enter_boot_preop(fsm);
            break;
        default: doFullStateMachine = 1; break;
        }
        break;
    default:
        doFullStateMachine = 1;
        break;
    }
    if (doFullStateMachine == 1) {
