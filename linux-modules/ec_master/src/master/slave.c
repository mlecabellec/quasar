...
/** CUSTOM : FORCED SYNC MANAGER COUNTER
 *
 *
 */

#ifdef EC_MASTER_SYNC_CONFIG_CAP
void ec_slave_set_fsync_count(ec_slave_t* slave,uint8_t count) 
{
	int i, rank;
	
	if( (slave->master->forcedSlavesSyncs==NULL) && (count>0) )
	{
		slave->master->forcedSlavesSyncs=kmalloc(sizeof(forced_slave_syncs_t),GFP_KERNEL);
		slave->master->countForcedSlaves=0;
	}
	
	rank=-1;
	for(i=0; i < slave->master->countForcedSlaves;++i)
	{
		if(slave->ring_position == slave->master->forcedSlavesSyncs[i].slave_index)
		{
			rank=i;
			slave->master->forcedSlavesSyncs[i].forced_sync_count=0;
			slave->master->forcedSlavesSyncs[i].isCompletelyConfigured=false;
			if(slave->master->forcedSlavesSyncs[i].isConfigured!=NULL)
			{
				kfree(slave->master->forcedSlavesSyncs[i].isConfigured);
				slave->master->forcedSlavesSyncs[i].isConfigured=NULL;
			}
			if(slave->master->forcedSlavesSyncs[i].forced_syncs!=NULL)
			{
				kfree(slave->master->forcedSlavesSyncs[i].forced_syncs);
				slave->master->forcedSlavesSyncs[i].forced_syncs=NULL;
			}
			break;
		}
	}
	...
	if(count>0)
	{
		if(rank==-1)
		{
			rank=slave->master->countForcedSlaves;
			slave->master->countForcedSlaves++;
			slave->master->forcedSlavesSyncs=krealloc(slave->master->forcedSlavesSyncs, sizeof(forced_slave_syncs_t) * slave->master->countForcedSlaves, GFP_KERNEL);
			slave->master->forcedSlavesSyncs[rank].slave_index=slave->ring_position;
			slave->master->forcedSlavesSyncs[rank].isCompletelyConfigured=false;
		}
		slave->master->forcedSlavesSyncs[rank].forced_sync_count=count;
		slave->master->forcedSlavesSyncs[rank].isConfigured=kmalloc(count * sizeof(bool), GFP_KERNEL);
		slave->master->forcedSlavesSyncs[rank].forced_syncs=kmalloc(count * sizeof(ec_sync_t), GFP_KERNEL);
		for(i=0; i<count; ++i)
		{
			slave->master->forcedSlavesSyncs[rank].isConfigured[i]=false;
			ec_sync_init(&(slave->master->forcedSlavesSyncs[rank].forced_syncs[i]),slave);
		}
	}
...
/** CUSTOM : FORCED SYNC MANAGER
 *
 *
 */

void ec_slave_set_fsync(ec_slave_t* slave, 
                    uint8_t index, uint16_t physical_start_address, 
                    uint16_t default_length, uint8_t control_register,
                    uint8_t enable) 
{
	int i, rank;

	rank=-1;
	if(slave->master->countForcedSlaves>0)
	{		
		for(i=0; i<slave->master->countForcedSlaves; ++i)
		{
			if(slave->master->forcedSlavesSyncs[i].slave_index==slave->ring_position)
			{
				rank=i;
				break;
			}
	...
	if(rank==-1)
	{
		EC_MASTER_WARN(slave->master, "Ignored forced sync config for slave %d, forced sync count has not been set", slave->ring_position);
	}
	else
	{
		if(index < slave->master->forcedSlavesSyncs[rank].forced_sync_count)
		{
			slave->master->forcedSlavesSyncs[rank].forced_syncs[index].physical_start_address=physical_start_address;
			slave->master->forcedSlavesSyncs[rank].forced_syncs[index].default_length=default_length;
			slave->master->forcedSlavesSyncs[rank].forced_syncs[index].control_register=control_register;
			slave->master->forcedSlavesSyncs[rank].forced_syncs[index].enable=enable;
			slave->master->forcedSlavesSyncs[rank].isConfigured[index]=true;
			slave->master->forcedSlavesSyncs[rank].isCompletelyConfigured=true;
			for(i=0; i<slave->master->forcedSlavesSyncs[rank].forced_sync_count; ++i)
			{
				if(!slave->master->forcedSlavesSyncs[rank].isConfigured[i])
				{
					slave->master->forcedSlavesSyncs[rank].isCompletelyConfigured=false;
					break;
				}
			}
			EC_MASTER_INFO(slave->master, "Sync configuration has been forced for slave %d, index : %d", slave->ring_position, index);
		}
		else
		{
			EC_MASTER_WARN(slave->master, "Ignored forced sync config for slave %d, syncIndex (%d) is above allowed sync count", slave->ring_position, index);
		}
	}
}
#endif
