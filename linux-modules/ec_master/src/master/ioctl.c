...
static ATTRIBUTES int ec_ioctl_slave_set_fsync_count(ec_master_t* master,
                                                    void *arg) {
    ec_ioctl_slave_set_fsync_count_t data;
    ec_slave_t *slave;
    ...
    ec_slave_set_fsync_count(slave,data.count);
    up(&master->master_sem);
    return 0;
}
static ATTRIBUTES int ec_ioctl_slave_set_fsync(ec_master_t* master,
                                                    void *arg) {
    ec_ioctl_slave_set_fsync_t data;
    ec_slave_t *slave;
    ...
    ec_slave_set_fsync(slave,data.index,data.physical_start_address,
                        data.default_length,data.control_register,data.enable);
    up(&master->master_sem);
    return 0;
}
        ...
        )
{
    ...
    switch (cmd) {
        ...
        case EC_IOCTL_SET_SEND_INTERVAL:
            ...
            break;
#ifdef EC_MASTER_SYNC_CONFIG_CAP
        case EC_IOCTL_SLAVE_SET_FSYNC_COUNT:
            if (!ctx->writable) {
                ret = -EPERM;
                break;
            }
            ret = ec_ioctl_slave_set_fsync_count(master, arg);
            break;
        case EC_IOCTL_SLAVE_SET_FSYNC:
            if (!ctx->writable) {
                ret = -EPERM;
                break;
            }
            ret = ec_ioctl_slave_set_fsync(master, arg);
            break;
#endif
        default:
            ret = -ENOTTY;
