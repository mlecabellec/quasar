 ...
 *  - Every 5 seconds: toggles a walking-bit pattern on the EL2809 digital outputs.
 ...
 *   [0x34..0x53]  EL3318  : 8 x 4-byte (2-byte status + 2-byte INT16 temp x 0.1°C)
 *   [0x54..0x63]  EL3314  : 4 x 6-byte (2-byte status + 2-byte INT16 temp + 2 align)
 *
 *   Total: 60 bytes (matches "Process Image size: 60 bytes" in test.log)
 *
 * Per ETG.1000.6 Beckhoff documentation:
 *  - EL3318/EL3314 temperature value: INT16, factor 0.1°C/LSB, range -200..+870°C
 ...
 *
 * @note Process image offsets are derived at runtime from slaves_[].outputs_offset
 *       and slaves_[].inputs_offset as set by configure_fmmu. The constants below
 *       are cross-checked against the test.log layout at startup. If the bus
 *       topology differs, the program aborts with a clear error.
 *
 * References:
 *  - Beckhoff EL2809: https://www.beckhoff.com/en-en/products/i-o/ethercat-terminals/el2xxx-digital-output/el2809.html
 *  - Beckhoff EL1809: https://www.beckhoff.com/en-en/products/i-o/ethercat-terminals/el1xxx-digital-input/el1809.html
 *  - Beckhoff EL3318: https://www.beckhoff.com/en-en/products/i-o/ethercat-terminals/el3xxx-analog-input/el3318.html
 *  - Beckhoff EL3314: https://www.beckhoff.com/en-en/products/i-o/ethercat-terminals/el3xxx-analog-input/el3314.html
...
constexpr uint16_t WKC_LRW_OUTPUT_INCREMENT = 1U; ///< Slave reads (drives outputs)
constexpr uint16_t WKC_LRW_INPUT_INCREMENT  = 2U; ///< Slave writes (drives inputs)

// Cyclic loop period.
constexpr std::chrono::milliseconds CYCLE_PERIOD_MS{10};

// Display/toggle interval: every 5 seconds = 500 cycles @ 10ms.
constexpr uint32_t DISPLAY_EVERY_N_CYCLES = 500U;
 ...
 */
static double read_el33xx_temperature(const ProcessImage &img, uint32_t base_offset) {
 ...
 * @details Byte 0 of the 2-byte status word holds error/underrange/overrange bits.
 * @param img         The process image buffer.
 * @param base_offset Byte offset of the start of this channel's record.
 * @return Low byte of the status word.
 */
static uint8_t read_el33xx_status(const ProcessImage &img, uint32_t base_offset) {
 ...
 */
static bool validate_topology(const Enumerator &enumerator, uint32_t image_bytes) {
    ...
    if (slaves.size() != EXPECTED_SLAVE_COUNT) {
        std::cerr << "[FATAL] Expected " << EXPECTED_SLAVE_COUNT
                  << " slaves, found " << slaves.size()
                  << ". Check bus topology." << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < EXPECTED_SLAVE_COUNT; ++i) {
        ...
        if (slaves[i].name.find(expected_names[i]) == std::string::npos) {
            std::cerr << "[FATAL] Slot " << i << ": expected '"
                      << expected_names[i] << "', found '"
                      << slaves[i].name << "'." << std::endl;
            return false;
        }
    }
    ...
    if (slaves[SLAVE_EL2809].outputs_size_bits == 0) {
        std::cerr << "[FATAL] EL2809 reports no output bits. Check SII parsing." << std::endl;
        return false;
    }

    // Validate EL1809 has inputs.
    if (slaves[SLAVE_EL1809].inputs_size_bits == 0) {
        std::cerr << "[FATAL] EL1809 reports no input bits. Check SII parsing." << std::endl;
        return false;
    }
    ...
    for (size_t i = 0; i < EXPECTED_SLAVE_COUNT; ++i) {
        std::cout << "  Slot " << i
                  ...
                  << " in_bits=" << slaves[i].inputs_size_bits
                  << " out_off=0x" << std::hex << slaves[i].outputs_offset
                  << " in_off=0x" << slaves[i].inputs_offset
                  << std::dec << std::endl;
    }
    return true;
}
...
static void display_digital_outputs(const ProcessImage &img, uint32_t offset) {
    ...
    std::cout << "[DO] EL2809 16x Digital Output (current pattern=0x"
              << std::hex << std::setw(4) << std::setfill('0') << do_word
...
static void display_el3318(const ProcessImage &img, uint32_t offset) {
    ...
    for (uint32_t ch = 0; ch < EL3318_CHANNELS; ++ch) {
        // Each channel record is 4 bytes wide (status_lo, status_hi, temp_lo, temp_hi).
...
int main(int argc, char *argv[]) {
    ...
    std::cout << "==========================================================" << std::endl;
    std::cout << " testmaster100 — EtherCAT Master for Beckhoff Demo Bus"    << std::endl;
    ...
    std::cout << "  EL2809 outputs at byte 0x" << std::hex << el2809_out_offset << std::dec << std::endl;
    std::cout << "  EL1809 inputs  at byte 0x" << std::hex << el1809_in_offset  << std::dec << std::endl;
    std::cout << "  EL3318 inputs  at byte 0x" << std::hex << el3318_in_offset  << std::dec << std::endl;
    std::cout << "  EL3314 inputs  at byte 0x" << std::hex << el3314_in_offset  << std::dec << std::endl;
    ...
    for (const SlaveInfo &s : slaves) {
        if (s.outputs_size_bits > 0U) { expected_wkc += WKC_LRW_OUTPUT_INCREMENT; }
        if (s.inputs_size_bits  > 0U) { expected_wkc += WKC_LRW_INPUT_INCREMENT; }
    }
    ...
    std::cout << "  Cycle: " << CYCLE_PERIOD_MS.count() << " ms | "
              << "Display: every " << (DISPLAY_EVERY_N_CYCLES * CYCLE_PERIOD_MS.count() / 1000)
              << " s" << std::endl;
    std::cout << "==============================================" << std::endl;
    ...
    for (uint64_t guard = 0U; guard < 9000000000ULL && g_stop == 0; ++guard) {
        ...
        if (!ex || *ex != expected_wkc) {
            ...
            if (wkc_errors % 100U == 1U) {
                std::cerr << "[WARN] WKC mismatch: got "
                          << (ex ? static_cast<int>(*ex) : -1)
                          << ", expected " << expected_wkc
                          << " (consecutive errors=" << wkc_errors << ")"
                          << std::endl;
            }
        } else {
        ...
        std::chrono::microseconds remaining =
            std::chrono::duration_cast<std::chrono::microseconds>(CYCLE_PERIOD_MS) - elapsed;
    ...
    // -----------------------------------------------------------------------
    std::cout << std::endl;
    std::cout << "========== Stop requested — shutting down ==========" << std::endl;
