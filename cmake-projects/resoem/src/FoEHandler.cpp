...
Result<> FoEHandler::write_file(SlaveInfo &slave, std::string_view filename,
  ...
  // Continue sending while there is data, or if the last packet was exactly max_payload
  // (a zero-length packet is needed to terminate if the total size is a multiple of max_payload).
