...
Result<> EoEHandler::send_frame(SlaveInfo &slave, std::span<const byte> frame,
  ...
  while (sent < frame.size()) {
    ...
    // info1 contains: Fragment number, Last fragment flag, and EoE Type (FRAME_DATA).
    h->info1 = eoe::make_info1(eoe::FRAME_DATA, frag, last, false);
    // info2 contains: Total frame size (for first fragment) or Offset (for subsequent ones).
...
Result<std::vector<byte>> EoEHandler::receive_frame(SlaveInfo &slave,
  ...
  // Keep reading mailbox fragments until the 'last' fragment of an Ethernet frame is reached.
  while (true) {
    size_t len;
    mailbox::Type type;
    ...
    // Verify that we are receiving a frame data fragment and it matches the expected sequence.
