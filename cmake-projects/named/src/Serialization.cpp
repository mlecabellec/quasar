...
// --- Helper Factory ---
/**
 * @brief Reconstructs a specific NamedObject derivative based on type information.
 */
std::shared_ptr<NamedObject> createFromTypeAndValue(const std::string& name, const std::string& type, const std::string& valueStr, std::shared_ptr<NamedObject> parent) {
    if (type == "Boolean") {
        ...
        // BitBuffer reconstruction. Currently assumes byte-aligned data from string.
...
void serializeToXml(XMLElement *element,
  ...
  // Use dynamic_cast to identify the underlying core type and serialize its value.
