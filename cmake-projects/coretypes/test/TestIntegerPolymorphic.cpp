...
TEST(IntegerPolymorphicTest, MixedWithFloatingPoint) {
    std::shared_ptr<Number> i = std::make_shared<Integer<int64_t>>(100);
    std::shared_ptr<Number> f = std::make_shared<FloatingPoint<double>>(1.5);
...
TEST(IntegerPolymorphicTest, OverflowChecks) {
    std::shared_ptr<Number> max_u64 = std::make_shared<Integer<uint64_t>>(std::numeric_limits<uint64_t>::max());
    std::shared_ptr<Number> one = std::make_shared<Integer<uint64_t>>(1);
    EXPECT_THROW(max_u64->safeAdd(*one), std::overflow_error);
    
    std::shared_ptr<Number> min_i64 = std::make_shared<Integer<int64_t>>(std::numeric_limits<int64_t>::min());
    std::shared_ptr<Number> neg_one = std::make_shared<Integer<int64_t>>(-1);
...
TEST(IntegerPolymorphicTest, BitwiseNotAndShifts) {
    ...
    std::shared_ptr<Number> shift4 = std::make_shared<Integer<int>>(4);
    std::shared_ptr<Number> shift2 = std::make_shared<Integer<int>>(2);
