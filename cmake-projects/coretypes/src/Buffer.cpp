...
Buffer::Buffer(const Buffer &other) : std::enable_shared_from_this<Buffer>() {
  ...
  if (!lock.owns_lock()) {
    throw std::runtime_error("Timeout acquiring lock on source buffer");
  }
  data_ = other.data_;
}
...
Buffer &Buffer::operator=(const Buffer &other) {
  if (this != &other) {
    // Fulfills [FE-0010.3.9] The class is thread safe.
    // Avoid deadlock by using address-based ordering for mutex acquisition.
    ...
    if (!lock1.owns_lock()) throw std::runtime_error("Timeout acquiring buffer lock (1)");
    std::unique_lock<std::recursive_timed_mutex> lock2(*second, std::chrono::seconds(1));
    if (!lock2.owns_lock()) throw std::runtime_error("Timeout acquiring buffer lock (2)");
...
Buffer Buffer::concat(const Buffer &other) const {
  ...
  if (this == &other) {
    // Special case for concatenating a buffer with itself.
    std::unique_lock<std::recursive_timed_mutex> lock(mutex_, std::chrono::seconds(1));
    if (!lock.owns_lock()) throw std::runtime_error("Timeout acquiring buffer lock");
  ...
  // Combine data from both buffers into a new instance.
  std::vector<uint8_t> new_data = data_;
