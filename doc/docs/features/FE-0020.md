...
- [FE-0020.1] Provide a "NamedObject" class.
  ...
  - [FE-0020.4] For each class of namespace quasar::coretypes, a derivated class shall be provided which shall also be a derivative of the NamedObject class.
    - [FE-0020.4.1] Slices (`NamedBufferSlice`, `NamedBitBufferSlice`) shall support construction directly from existing `NamedBuffer`/`NamedBitBuffer` instances or other slices.
    - [FE-0020.4.2] Slices (`NamedBufferSlice`, `NamedBitBufferSlice`) shall enforce out-of-bounds safety upon construction and throw `std::out_of_range`.
  ...
  - [FE-0020.14] Utilities shall be provided for copying, moving, and removing parts of the tree (children, siblings, ancestors, descendants).
    - [FE-0020.14.1] Copying capabilities shall include a deep copy mechanism (`deepCopy()`) that duplicates the entire tree topology recursively.
    - [FE-0020.14.2] The deep copy mechanism shall correctly rebase slice objects (`NamedBufferSlice`, `NamedBitBufferSlice`) pointing to buffers within the copied tree, preserving relative offsets.
  - [FE-0020.18] Provide a declarative, rule-based Tree Transformation Engine.
    - [FE-0020.18.1] Support out-of-place (functional) transformation producing new hierarchies.
    - [FE-0020.18.2] Support in-place (mutating) transformation for efficient tree morphing.
    - [FE-0020.18.3] Rules shall consist of a Predicate (matcher) and a Generator (morphed node producer).
    - [FE-0020.18.4] Support prioritized rule evaluation.
    - [FE-0020.18.5] Ensure thread-safety for concurrent in-place transformations on non-overlapping branches.
  - [FE-0020.19] Support Zero-Copy Reinterpretation of memory buffers into named primitives.
    - [FE-0020.19.1] Leaf nodes (NamedInteger, NamedFloatingPoint, NamedBoolean) shall be capable of binding to contiguous memory ranges within a parent buffer.
    - [FE-0020.19.2] Operations on bound primitives (getValue, setValue) shall directly interact with the backing buffer memory.
    - [FE-0020.19.3] Support explicit endianness specification for bound multi-byte primitives.
